<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ taglib prefix="c" uri="jakarta.tags.core" %>
<c:set var="role" value="${sessionScope.adminUser.role}" />
<c:set var="cp"   value="${pageContext.request.contextPath}" />

<aside class="admin-sidebar">
    <div class="sidebar-header">
        <a href="${cp}/admin/dashboard" class="sidebar-logo">
            <img src="https://i.postimg.cc/5N6zh927/logo-white.png" alt="Logo">
        </a>

    </div>

    <ul class="admin-menu">


        <c:if test="${role == 'ADMIN' || role == 'MANAGER' || role == 'STAFF_ORDER'}">
        <li>
            <a href="${cp}/admin/dashboard" data-page="dashboard">
                <i class="fas fa-home"></i>
                <span>Dashboard</span>
            </a>
        </li>


        <li>
            <a href="${cp}/admin/notifications" data-page="notifications">
                <i class="fas fa-bell"></i>
                <span>Thông báo</span>
            </a>
        </li>
        </c:if>


        <c:if test="${role == 'ADMIN' || role == 'MANAGER' || role == 'STAFF_ORDER'}">
        <div class="menu-section-title">Quản lý bán hàng</div>
        </c:if>


        <c:if test="${role == 'ADMIN' || role == 'MANAGER'}">
        <li>
            <a href="${cp}/admin/products" data-page="products">
                <i class="fas fa-box"></i>
                <span>Sản phẩm</span>
            </a>
        </li>
        <li>
            <a href="${cp}/admin/flash-sales" data-page="flash-sales">
                <i class="fas fa-bolt"></i>
                <span>Flash Sale</span>
            </a>
        </li>
        <li>
            <a href="${cp}/admin/coupons" data-page="coupons">
                <i class="fas fa-ticket-alt"></i>
                <span>Mã giảm giá</span>
            </a>
        </li>
        <li>
            <a href="${cp}/admin/categories" data-page="categories">
                <i class="fas fa-list"></i>
                <span>Danh mục</span>
            </a>
        </li>
        </c:if>


        <c:if test="${role == 'ADMIN' || role == 'MANAGER' || role == 'STAFF_ORDER'}">
        <li>
            <a href="${cp}/admin/orders" data-page="orders">
                <i class="fas fa-shopping-cart"></i>
                <span>Đơn hàng</span>
            </a>
        </li>
        <li>
            <a href="${cp}/admin/reviews" data-page="reviews">
                <i class="fas fa-star"></i>
                <span>Đánh giá</span>
            </a>
        </li>
        <li>
            <a href="${cp}/admin/refund-requests" data-page="refund-requests">
                <i class="fas fa-undo-alt"></i>
                <span>Hoàn tiền</span>
            </a>
        </li>
        <li>
            <a href="${cp}/admin/contacts" data-page="contacts">
                <i class="fas fa-envelope"></i>
                <span>Liên hệ</span>
            </a>
        </li>
        <li>
            <a href="${cp}/admin/chat" data-page="chat">
                <i class="fas fa-comments"></i>
                <span>Chat CSKH</span>
            </a>
        </li>
        </c:if>


        <c:if test="${role == 'ADMIN'}">
        <li>
            <a href="${cp}/admin/users" data-page="customers">
                <i class="fas fa-users"></i>
                <span>Khách hàng</span>
            </a>
        </li>
        </c:if>


        <c:if test="${role == 'ADMIN' || role == 'MANAGER' || role == 'STAFF_CONTENT'}">
        <div class="menu-section-title">Nội dung</div>
        <li>
            <a href="${cp}/admin/posts" data-page="posts">
                <i class="fas fa-newspaper"></i>
                <span>Bài viết</span>
            </a>
        </li>
        <li>
            <a href="${cp}/admin/static-pages" data-page="static-pages">
                <i class="fas fa-file-alt"></i>
                <span>Trang tĩnh</span>
            </a>
        </li>
        </c:if>


        <div class="menu-section-title">Hệ thống</div>
        <li>
            <a href="${cp}/admin/logout">
                <i class="fas fa-sign-out-alt"></i>
                <span>Đăng xuất</span>
            </a>
        </li>

    </ul>
</aside>